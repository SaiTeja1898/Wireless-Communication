...
for P_dB=SNR_dB
    ...
    for sample = 1:10^6
        ...
        %achieving snr by sig pwr=1 and noise pwr=1/P
        gamma1=(abs(h_nu)^2);
        gamma2=(abs(h_fu)^2);
        optimal_alpha=(-(gamma1+gamma2)+sqrt(((gamma1+gamma2)^2)+4*gamma1*(gamma2^2)*P))/(2*gamma1*gamma2*P);
        %Superposition
        x_noma=sqrt(optimal_alpha*P)*x_nu+sqrt((1-optimal_alpha)*P)*x_fu;
        ...
        mod_rx_fu_at_nu=h_nu*sqrt((1-optimal_alpha)*P)*rx_fu_at_nu;
...
semilogy(SNR_dB,ABER_nu,'-*k');
hold on
grid on
semilogy(SNR_dB,ABER_fu,'-*b');
legend('Near User optimal','Far User optimal');